...
class AlertNotifications
{
    ...
    public function isParentDown(int $device_id): bool
    {
        ...
        $down_parent_count = DB::table('devices as d')
            ->leftJoin('devices_attribs as a', 'd.device_id', '=', 'a.device_id')
            ...
            ->where('d.disabled', 0)
            ->where(function ($query) {
                $query->where('d.status_reason', 'icmp')
                    ->orWhere(function ($query) {
                        $query->where('a.attrib_type', 'override_icmp_disable')
                            ->where('a.attrib_value', 'true');
                    });
            })
            ->count();
