...
$('#poller-groups').on('show.bs.modal', function (event) {
    ...
    if(group_id != '') {
        ...
        $.ajax({
            type: "GET",
            url: '{{ route("pollergroup.show", ["pollergroup" => "PLACEHOLDER"]) }}'.replace('PLACEHOLDER', group_id),
            dataType: "json",
...
$('#create-group').on("click", function(e) {
    ...
    var url = group_id ? '{{ route("pollergroup.update", ["pollergroup" => "PLACEHOLDER"]) }}'.replace('PLACEHOLDER', group_id) : '{{ route("pollergroup.store") }}';
    var type = group_id ? 'PUT' : 'POST';
    $.ajax({
        type: type,
        url: url,
        data: { group_name: group_name, descr: descr, _token: '{{ csrf_token() }}' },
        dataType: "html",
        success: function(msg){
