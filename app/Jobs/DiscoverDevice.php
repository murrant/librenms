...
EOH, $this->device->hostname, $os_group ? " ($os_group)" : '', $this->device->device_id, $this->device->os, $this->device->ip));
    ...
    private function handleOsChange(OS $os): OS
    {
        ...
        // Set type to a predefined type for the OS if it's not already set (still could be overridden later by os discovery)
        $loaded_os_type = LibrenmsConfig::getOsSetting($this->device->os, 'type');
        if (! $this->device->getAttrib('override_device_type') && $loaded_os_type != $this->device->type) {
            $this->device->type = $loaded_os_type;
            $this->deviceArray['type'] = $loaded_os_type;
        }
