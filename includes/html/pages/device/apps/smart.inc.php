...
// Build drive navigation menu
$driveLinks = [];
foreach ($disks as $diskName => $diskData) {
    $label = $diskName;
    $isSelected = isset($vars['disk']) && $vars['disk'] === $diskName;
...
if (isset($vars['disk'])) {
    ...
    if (! $isLegacy && ! empty($currentDisk)) {
        ...
        if (isset($currentDisk['selftest_log'])) {
            $formattedLog = str_replace('n#', "\n#", $currentDisk['selftest_log']);
            echo "<pre>{$formattedLog}</pre><br>\n";
        }
    ...
    // Remove SSD graphs for non-SSD disks
    if (($currentDisk['is_ssd'] ?? 0) !== 1) {
        unset($graphs['smart_ssd']);
    }
} else {
    // All disks view - show graphs based on available SMART attributes
    $smartAttributes = [
    ...
    foreach ($smartAttributes as $attribute => $graphInfo) {
        if (($hasData[$attribute] ?? 0) === 1) {
            [$graphKey, $graphLabel] = $graphInfo;
            $graphs[$graphKey] = $graphLabel;
        }
    }

    // Special case: Max temperature graph if either temp attribute exists
